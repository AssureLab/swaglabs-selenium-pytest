...
def test_problem_user_login(browser, config):
    ...
    img = wait_for(browser).until(
        EC.visibility_of_element_located((LoginPageLocators.IMG_BACKPACK))
    )
    src = img.get_attribute("src")
    assert (
        "sauce-backpack-1200x1500" in src
    ), f"Backpack image is incorrect for problem_user: {src}"
...
def test_glitch_user_login(browser, config):
    ...
    assert (
        load_time > 2
    ), f"Expected glitch delay, but load time was {load_time:.2f} seconds"
